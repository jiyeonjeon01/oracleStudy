...
-- 임금(pay) 별 등수 처리 프로시저
CREATE OR REPLACE PROCEDURE PARTTIME_RANK_PROC
IS 
    VPARTTIME_RT PARTTIME%ROWTYPE;
    CURSOR C1 IS 
    SELECT ID, NAME, PAY, RANK() OVER (ORDER BY PAY DESC)AS RANK FROM PARTTIME ORDER BY PAY DESC; 
BEGIN
     FOR VPARTTIME_RT IN C1 LOOP
        UPDATE PARTTIME SET RANK = VPARTTIME_RT.RANK WHERE ID = VPARTTIME_RT.ID;
        COMMIT;
     END LOOP;
     COMMIT;
 END;
